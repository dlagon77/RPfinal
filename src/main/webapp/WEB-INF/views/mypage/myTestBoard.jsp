...
<script>
	...
	function detail(sno){
		location.href = 'myTestDetailView.do?sno='+sno;
	}
</script>	
<!-- === BEGIN CONTENT === -->
...
<!-- 								<th class="qtable" style="width:100px;">해당수업</th> -->
							...
							<c:if test="${result ne 0 }">
								<c:forEach var="sc" items="${list }">
									<tr onclick="detail(${sc.test_no });"
										style="cursor: pointer">
<%-- 										<td style="text-align: center;">${sc.test_no}</td> --%>
										<td style="text-align: center;">${sc.mem_name}</td>
										<td style="text-align: center;">${sc.test_cate}</td>
										<td style="text-align: center;">${sc.test_title}</td>
										<td style="text-align: center;">${sc.test_date}</td>
				...
				<%-- 	 	<c:url var="stusub" value="stumypage.do">
						...
						<div class="text-center">
							<ul class="pagination">
								...
								<c:if test="${currentPage > 1 }">
									<c:url var="paging1" value="myTestBoard.do">
								...
								<c:forEach begin="${startPage}" end="${endPage}" step="1" varStatus="status">
									...
									<c:url var="paging2" value="myTestBoard.do">
								...
								<c:if test="${currentPage < maxPage }">
									<c:url var="paging3" value="myTestBoard.do">
                          				 <c:param name="page" value="${currentPage+1 }" />
                          				 <c:param name="mem_no" value="${sessionScope.loginUser.mem_no }" />
                        			</c:url> 
									<li><a
 				...
 				<form action="/finalp/myTestBoard.do" method="get" style="display:inline-flex;margin-left: 470px;">
					<div align="right" style="width:200px;">
						<c:if test="${keyword ne ''}">
							 <c:url var="keyword1" value="myTestBoard.do">
			                          	<c:param name="mem_no" value="${sessionScope.loginUser.mem_no }" />
			               	</c:url> 
