package com.rp.finalp.common.util;
...
public class FileUtils {
	...
	public String InsertFile(HttpSession session, HttpServletRequest request) throws Exception{
    	...
    	String renamedFileName = null;	// 변환한 파일명("랜덤텍스트.확장자")
        ...
        if(iterator.hasNext()){
        	...
        	if(multipartFile.isEmpty() == false){	// multipartFile이 비어있지 않으면
        		...
        		// 저장되는 파일명 : "랜덤텍스트.확장자"
        		renamedFileName = CommonUtils.getRandomString() + originalFileExtension;
        		
        		file = new File(filePath + renamedFileName);	// 파일 객체 만들고,
        		multipartFile.transferTo(file);	// 실제 파일을 만들어준다.
        	}
        }
		return new String(originalFileName + "/" + renamedFileName);	// 변환한 파일명을 반환
	}
